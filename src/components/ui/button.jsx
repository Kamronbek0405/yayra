...
const Button = ({
  ...
  variant,
  size,
  startIcon: StartIcon, // Destructure icons
  endIcon: EndIcon,
  asChild = false,
  children,
  ...props
  ...
  return (
    <Comp className={cn(buttonVariants({ variant, size, className }))} {...props}>
      {StartIcon && <span className="mr-2">{<StartIcon className="h-4 w-4" />}</span>} {/* Render StartIcon */}
      {children}
      {EndIcon && <span className="ml-2">{<EndIcon className="h-4 w-4" />}</span>} {/* Render EndIcon */}
    </Comp>
  );
};
