    ...
    <div>
      {blogData?.slice(0,1).map((item) => (
        <div className=' ml-8 lg:px-20 h-auto   flex items-center justify-between flex-col lg:gap-10'>
           ...
           <h1 className=' text-2xl md:text-3xl lg:text-4xl font-medium py-5'>{item.text}</h1>
            <Button className={" ml-10 py-5 md:px-6 md:py-6 lg:px-8 lg:py-6 "}>{item.topright}</Button>
           </div>
          ...
          <div className=' flex flex-col   md:flex md:flex-row  gap-7  lg:flex lg:flex-row lg:gap-10 mt-7 '>
            {blogData?.slice(0,2).map((item) => (
              <div className='flex flex-col  lg:flex-none'>
                <img className=' w-[320px] h-[240px] md:w-[350px] md:h-[260px] lg:w-[392px] lg:h-[281px] ' src={item.img} alt="img" />
                <span className='text-gray-500 mt-5'>{item.day}</span>
                <h1 className='mt-5 text-lg  w-[320px] md:text-xl md:w-[350px] lg:text-xl font-medium lg:w-[400px]'>{item.title}</h1>
                <p className='mt-3  w-[320px]  text-gray-500 lg:w-[350px]'>{item.description}</p>
              </div>
            ))}
           </div>
            ...
            {blogData?.slice(2).map((item) => (
              <div>
                ...
                <div>
                  ...
                  <p className=' w-[220px] md:text-lg md:w-[300px] lg:text-lg  font-medium lg:w-[300px]'>{item.title}</p>
                </div>
              </div>
